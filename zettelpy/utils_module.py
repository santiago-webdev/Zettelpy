...
# Actual date in long format with suffix on the number of the day
def pretty_date() -> str:
...
def first_actions(flag_is_last: bool, flag_title: Optional[Path] = None) -> str or None:
    """
    ...
    if flag_is_last is True and flag_title is not None:
        print("Flag --last present, ignoring title")
        with open("last_note", "r") as last_note:
            return last_note.read().rstrip("\n")
    elif flag_is_last is True:
        with open("last_note", "r") as last_note:
            return last_note.read().rstrip("\n")
    elif flag_is_last is False and flag_title is not None:
        with open("last_note", "w") as last_accessed:
            flag_title = check_extension(flag_title)  # Make flag_perm a .md file
            main_arg_check = Path("permanent", flag_title)  # Correct the path
            last_accessed.write(str(main_arg_check))  # And write the Path to the file
            return main_arg_check
    elif flag_is_last is False and flag_title is None:
        return None
    else:
